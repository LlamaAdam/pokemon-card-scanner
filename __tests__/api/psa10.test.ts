...
vi.mock('@/lib/kvClient', () => ({
  getKv: () => ({
    ...
    set: vi.fn(async (k: string, v: unknown) => { kvStore.set(k, v); }),
  }),
}));
...
import handler from '@/pages/api/psa10';
...
function jsonPayload(res: NextApiResponse): unknown {
...
describe('/api/psa10', () => {
  ...
  it('returns cached price without fetching', async () => {
    ...
    const req = makeReq({ setCode: 'OBF', number: '125', cardName: 'Charizard ex' });
    const res = mockRes();
    await handler(req, res);
    expect(fetchMock).not.toHaveBeenCalled();
    expect(res.status).toHaveBeenCalledWith(200);
    expect(jsonPayload(res)).toMatchObject({ price: 480, cached: true });
  });
  ...
  it('scrapes, caches, and returns on cache miss', async () => {
    ...
    expect(jsonPayload(res)).toMatchObject({ price: 500, cached: false });
  ...
  it('caches negative result on parse miss', async () => {
    ...
    expect(jsonPayload(res)).toMatchObject({ price: null, reason: 'not_found' });
  });

  it('returns 404 when set code is not in SET_SLUG_MAP', async () => {
    const req = makeReq({ setCode: 'ZZZ', number: '001', cardName: 'Foo' });
    const res = mockRes();
    await handler(req, res);
